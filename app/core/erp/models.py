...
class Product(models.Model):
    ...
    def toJSON(self):
        # hay que tener cuidado que hay campos que con model_to_dict (me
        # devuelve un diccionario) no se pueden convertir directamente
        # (imagenes, fechas, relaciones con otras tablas...)
        # Puedo agregar los campos que yo quiero
        item = model_to_dict(self)
...
class Sale(models.Model):
    ...
    def delete(self, using = None, keep_parents = False):
        for det in self.detsale_set.all():
            # como tengo las relaciones puedo hacer esto
            det.prod.stock += det.cant
            det.prod.save()
        # al final se hace el delete en cascada
