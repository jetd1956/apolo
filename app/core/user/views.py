...
from core.printTicket import printTicket
...
class UserChangeGroup(LoginRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        try:
            grp =  Group.objects.get(pk=self.kwargs['pk'])
            request.session['group_id'] = grp.id
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            if action == 'edit':
                ...
                if form.is_valid():
                    form.save()
                    # para que no se cierre la sesion y se actualize la nueva clave en el
                    # usuario actual, escribimos la siguiente sentencia:
