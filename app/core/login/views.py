...
class ResetPasswordView(FormView):
    ...
    def send_email_reset_pwd(self, user):
        data = {}
        try:
            # si DEBUG esta en falso significa que estamos en PRoduccion
            # si no es produccion obtenemos la ruta donde estamos trabajando
    ...
    def post(self, request, *args, **kwargs):
        data = {}
        try:
            form = ResetPasswordForm(request.POST)  # o self.get_form()
            if form.is_valid():
                user = form.get_user()
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            if form.is_valid():
                ...
                # generamos nuevamente el token para invalidar la url enviada en el correo
                user.token = uuid.uuid4()
                user.save()
            else:
                data['error'] = form.errors
